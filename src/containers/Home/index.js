                ...
                <Row gutter={16}>
                  <Col span={6}>
                    <Avatar
                      shape="square"
                      size={100}
                      src={props.selectPeople.img || userIcon}
                    />
                  </Col>
                </Row>
                <Row gutter={16}>
                  <Col span={6}>Name</Col>
                  <Col span={10}>{props.selectPeople.name}</Col>
                </Row>
                <Row gutter={16}>
                  <Col span={6}>Id</Col>
                  <Col span={10}>{props.selectPeople.id}</Col>
                </Row>
                <Row gutter={16}>
                  <Col span={6}>Description</Col>
                  <Col span={10}>{props.selectPeople.Description}</Col>
                </Row>
              </div>
            )}
          </Col>
...
const mapDispatchToProps = dispatch => ({
  ...
  handleCheckrdPeople: (value, check) =>
    dispatch(Action.handleCheckrdPeople(value, check)),
  handleDelete: () => dispatch(Action.handleDelete())
});
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(presenter(homePM, Home));
